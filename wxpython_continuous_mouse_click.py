...
class MyFrame(wx.Frame):
    ...
    def control_panel_main(self):
        ...
        self.animation.LoadFile("Spinner-1s-30px.gif")
    ...
    async def count_and_handle_simulators(self, device):
        ...
        """
        try:
            running_name = subprocess.check_output(["adb", "-s", f"{device.serial}", "shell", "getprop", "ro.boot.qemu.avd_name"],
    ...
    def check_animation_status(self):
        """动态加载状态"""
        if self.selected_Android_option_name in self.devices_info and self.devices_info[self.selected_Android_option_name]:
            # 启动中、关闭中
        ...
        if selected_Android_option_name in self.devices_info:
            ...
            # 启动中
                ...
                if selected_Android_option_name in running_Android_name:
                    ...
                    self.devices_info[selected_Android_option_name]["server"] = self.devices[running_Android_name[selected_Android_option_name]]
    ...
    def click_simulator_control(self):
        ...
        if total_likes - 1 == self.current_click_num:
            if hasattr(self, "click_status_statictext"):
                self.click_status_statictext.SetLabel(f"第{batch_value}批点赞任务，已完成。成功：{total_likes - 1}个")
            else:
                self.click_status_statictext = wx.StaticText(parent=self.panel,
                                                             label=f"第{batch_value}批点赞任务，已完成。成功：{total_likes - 1}个",
                                                             pos=(10, 160))
        else:
            if hasattr(self, "click_status_statictext"):
                ...
                self.click_status_statictext = wx.StaticText(parent=self.panel,
                                                             label=f"第{batch_value}批点赞任务，未完成。失败：{self.current_click_num - total_likes}个",
                                                             pos=(10, 160))
